...
struct DECLARE_WAIT_QUEUE_HEAD(mywq);
dev_t cdev;
struct cdev my_cdev;
...
static int __init chr_driver_start(void)
{
	...
	if(alloc_chrdev_region(&cdev,0,1,"mutexlock")<0)
	{
	...
	if((cdev_add(&my_cdev,cdev,1)<0))
	{
		...
		k_class=class_create(THIS_MODULE,"hi_mutexlock");

	//device create
		device_create(k_class,NULL,cdev,NULL,"mutexlock");
