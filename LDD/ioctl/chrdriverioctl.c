...
{	
	...
	//copy_from_user(bu+*off,buff,len);//user to kernel
	//bu[i]=(char *)buff;
	*off=*off+len;
	return 0;
}
...
static int __init chr_driver_start(void)
{
	...
	if(alloc_chrdev_region(&cdev,0,1,"ioctl1")<0)
	{
	...
	if((cdev_add(&my_cdev,cdev,1)<0))
	{
		...
		k_class=class_create(THIS_MODULE,"hi_ioctl");

	//device create
		device_create(k_class,NULL,cdev,NULL,"ioctl1");
